...
def generate_bias(karyotype, low_to_high, high_to_low):
    p_low = high_to_low / (low_to_high + high_to_low)
    bias = []
    for chrom in karyotype:
        chrom_init = np.random.choice([0,1],p=[p_low,1-p_low])
        chrom_bias = [chrom_init]
        n = 1
        region_size = []
        while n < chrom:
            if n > 1:
                chrom_bias.append(1-chrom_bias[-1])
            if chrom_bias[-1] == 0:
                bias_region_size = stats.geom.rvs(low_to_high)
            elif chrom_bias[-1] == 1:
                bias_region_size = stats.geom.rvs(high_to_low)
            region_size.append(bias_region_size)
            n += bias_region_size
        region_size[-1] = region_size[-1] - (n - chrom - 1) 
        bias.append(np.vstack([np.array(chrom_bias),np.array(region_size)]).T)
    return bias

def sample_biased_cut_chrom(bias_chrom, xi, rng = None):
    chrom_size = bias_chrom[:,1].sum()
    
    plow = bias_chrom[bias_chrom[:,0] == 0][:,1].sum()
    phigh = xi * bias_chrom[bias_chrom[:,0] == 1][:,1].sum()
    Z = plow + phigh

    plow = plow / Z
    phigh = phigh / Z
    
    bias_factor = np.copy(bias_chrom[:,0])
    bias_factor = bias_factor * xi
    bias_factor[bias_factor == 0] = 1
    
    p_pick = np.cumsum(bias_chrom[:,1] * bias_factor)
    p_pick /= p_pick[-1]
    ...
    if rng == None:
        ...
        r = rng.random()
    region_choice = np.digitize(r,p_pick)

    cumulative_sum = np.cumsum(bias_chrom[:,1])
    left_bounds = np.array([0]+list(cumulative_sum[:-1]))

    left_bound = left_bounds[region_choice] + 1
    right_bound = left_bounds[region_choice] + bias_chrom[region_choice][1]
    if rng == None:
        cut_choice = np.random.choice(range(left_bound,right_bound+1))
    else:
        cut_choice = rng.choice(range(left_bound,right_bound+1))
    
    return cut_choice

# NOT UPDATED - CURRENTLY JUST COPY OF NEUTRAL
def simulate_nonneutral_genome_evolution(karyotype, T, kappa, gamma, theta, bias, return_trajectory = False):
    raise ValueError("Placeholder - non-neutral simulations not yet implemented.")
